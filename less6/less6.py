# 1.Создать функцию на проверку введённых данных:
#  - Проверка имени
#  - проверка возраста
# Функции должны возвращать строку с ошибкой.
# Если строки вернули ошибки, нужно вывести их пользователю.
# 2. Улучшить проверку имени, в которой допускается один пробел.
# 3. сделать совет по получению/замене паспорта в отдельной функции,
#     которая возвращает строку с ответом или не возвращает ничего.
# 4. Создать функцию main, в которой будет вызов всех остальных функций, ввод данных и прочее.
# 5. Создать цикл до тех пор, пока пользователь не введёт верные данные.
# 6. Создать функцию, которая очищает введенные данные от лишних пробелов.
# 7. Все функции должны иметь документацию и аннотации.
#
# !!! Два принта !!!
...
Если имя введено верно - ничего не выводим (или None).
    ...
    if not name:    # Проверяем сразу на пустую строку, иначе конфликт со 2 проверкой.
        ...
        return 'Ваше имя слишкм короткое!'
...
Если возраст соответствует критериям - выводим совет.
    ...
    if int(age) in range(16, 18):   # Если возраст равен 16 или 17.
        ...
        return 'Вам необходим заменить паспорт!'
...
Пропускаем (прерываем) цикл проверки.
    ...
    while True: # Бескончный цикл проверки, пока условие не будет == False.
        name = input('Введите Ваше имя: ').capitalize() # Вводим имя
